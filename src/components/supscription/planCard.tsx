...
const PlanCard: React.FC<Props> = ({ plan, onSelect }) => {
  ...
  return (
    ...
    >
      ...
      <button
        onClick={() => {
          if (!isFreePlan) onSelect(plan.id);
        }}
        disabled={isFreePlan}
        className={`mt-auto w-full font-semibold py-2 px-4 rounded transition-colors transform-gpu
          ${isFreePlan
            ? "bg-gray-400 text-white cursor-not-allowed"
            : "bg-blue-600 hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600 text-white group-hover:translate-y-[-2px]"
          }`}
      >
        {isFreePlan
          ? t("alreadySelected")
